    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status === 401 || error.response.status === 403) {
          ...
          setError(`Failed to load sales: ${error.response.data?.error || error.message}`);
        }
      ...
      <div className="my-orders-page">
        ...
        <div className="my-orders-container">
          <div className="error-state">
            <p>{error}</p>
            <button onClick={fetchSales} className="btn-refresh">Retry</button>
          </div>
        </div>
      </div>
    );
                ...
                <div className="order-main" onClick={() => setExpandedOrder(expandedOrder === order.orderId ? null : order.orderId)}>
                  ...
                  <div className="order-actions-col">
                    <button 
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(`/sales-order-details/${order.orderId}`);
                      }}
                    >
                      View
                    </button>
